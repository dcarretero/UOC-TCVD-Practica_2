...
### 4.1.1. Prediccion GLM
```{r}
dt_winequality.data.test.X<-dt_winequality.data.test%>%select(-c(quality_class))
dt_winequality.data.test.Y<-dt_winequality.data.test%>%select(c(quality_class))
predicted_GLM<- predict(modelGLM, dt_winequality.data.test)
```


### 4.1.2.Matriz confusión- NAIVE BAYES
```{r}
confusionMatrix(reference = df_test$Class, data = predictedNB, mode='everything', positive='positive')
```

### 4.1.3. AUC- NAIVE BAYES
```{r}
auc(roc(predictedNB,df_test$Class))
```
### 4.1.4. Accuracy y Kapppa - NAIVE BAYES

```{r}
postResample(predictedNB, df_test$Class)
```
